...
void addArrayManipulationNativeWords(Interpreter& interp) {
    interp.defineNativeWord(U"ALLC", allocOp);
    interp.defineNativeWord(U"COPY", copyOp);
    interp.defineNativeWord(U"DCOPY", deepCopyOp);
    interp.defineNativeWord(U"APPEND", appendOp);
    interp.defineNativeWord(U"RESZ", resizeOp);
}
